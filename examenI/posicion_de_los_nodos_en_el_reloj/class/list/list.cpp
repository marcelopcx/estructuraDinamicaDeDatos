...
template<class T>
void List<T>::clockPositions() {
   if (length < 4) {
      cout<<"No hay suficientes personas en el círculo."<<endl;
      return;
    }

   Node<T>* temp = head;
   int step = length / 4; // Dividir la longitud por 4 para encontrar los pasos entre cada "hora"

   cout<<"A las 12: "<< head->getData()<<endl;

   for (int i = 0; i < step; i++)
   {
      temp = temp->getNextNode();
   }
   cout<< "A las 3: "<< temp->getData()<<endl;

   for (int i = 0; i < step; i++)
   {
      temp = temp->getNextNode();
   }
   cout<< "A las 6: "<< temp->getData()<<endl;

   for (int i = 0; i < step; i++)
   {
      temp = temp->getNextNode();
   }
   cout<< "A las 9: "<< temp->getData()<<endl;
}